...
use super::slot32::Slot;
use anyhow::{Result, Ok};

pub struct QuotientFilter32 {
    count: usize,
    remainder: u8,
    size: usize,
    table: Vec<Slot>  
...
impl QuotientFilter32 {
    ...
    pub fn merge(&mut self, other: &QuotientFilter32) -> Result<()> {
...
mod tests {
    ...
    fn delete_read_multiple_value_multiple_success() {
        let mut filter = QuotientFilter32::new(10).unwrap();
        _ = filter.insert_value(&1_u8.to_be_bytes());
        _ = filter.insert_value(&2_u8.to_be_bytes());
        _ = filter.insert_value(&3_u8.to_be_bytes());
        _ = filter.insert_value(&4_u8.to_be_bytes());
    ...
    fn read_after_resize_one_element() {
        let mut filter = QuotientFilter32::new(2).unwrap();
        _ = filter.insert_value(&1_u8.to_be_bytes());
        _ = filter.resize();
