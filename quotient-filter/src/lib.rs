...
impl QuotientFilter {
    pub fn new(quotient: u8) -> Result<Self> {
        if quotient > 62 { return Err(anyhow::Error::new(QuotientFilterError::InvalidQuotientSize)); }
        let size = usize::pow(2, quotient as u32);
        let remainder = 64 - quotient;
