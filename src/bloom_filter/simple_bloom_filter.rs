...
impl SimpleBloomFilter {
    ...
    pub fn insert(&mut self, item: &[u8]) {
        println!("{:?}", self.number_of_hash_functions);
        for i in 0..self.number_of_hash_functions {
            let index = const_murmur3::murmur3_32(item, i as u32) as usize %  self.space as usize;
    ...
    pub fn lookup(&self, item: &[u8]) -> bool {
        for i in 0..self.number_of_hash_functions {
            let index = const_murmur3::murmur3_32(item, i as u32) as u64 %  self.space;
